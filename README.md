...
- Streamed text generations are owned and bounded by the authenticated desktop gateway. The mobile page transparently reattaches at its last received byte after a hidden/frozen WebView drops the HTTP connection, without PiP, overlays, wake locks, or a foreground-service notification. Explicit Stop actions still cancel the upstream request.
...
Right-click its taskbar hidden icon to open the verified SillyTavern desktop UI, open the authentication hub, start/stop the gateway for the current ST session, or toggle `Start with Windows`. The desktop-open action is enabled only for a live root-verified SillyTavern session and never launches a duplicate server. Logon startup is opt-in and only starts the tray watcher; it waits for SillyTavern rather than launching SillyTavern on its own. Gateway auto-start requires the trusted ST record published by ST Launcher option 1 (or by `Start-StMobile.ps1` when that script itself launches ST), so a lookalike `node server.js` process from another root is refused.

For streamed generations, the gateway owns the upstream request and retains a bounded byte-exact replay buffer. Android lock/background lifecycle signals detach only the phone-side transport; unlock, page visibility, focus, and network-return signals reattach the same relay at its last delivered byte without starting a second generation. A visible-state no-progress watchdog repairs silent half-open connections even when a pause signal was missed. The client retries for up to 80 minutes while active and completed server relays retain replay state for 90 minutes, subject to the documented 16 MiB per-relay and 64 MiB aggregate memory caps. Redacted gateway logs record relay creation, attachment offsets, detachment, completion, expiry, and capacity eviction without prompt or token content. SillyTavern's **Stop** action remains the separate idempotent path that cancels the upstream request.

This recovery does not use PiP, an overlay, a wake lock, or a foreground service. If Android kills only the WebView renderer, the app discards the dead WebView and reloads the paired UI in a fresh renderer. Whole-app process death or a page reload still destroys SillyTavern's in-page generation consumer; restoring and saving an in-progress generation after that stronger failure requires a separate SillyTavern UI-state reconciliation feature.

Rank Audit for phone-lock stream recovery:

- Rank 4: while the same SillyTavern page and Android process remain alive, phone lock, WebView suspension, focus return, and silent stale mobile transports must recover through the same server-owned relay ID and exact byte offset without duplicating or canceling the upstream generation; an actual SillyTavern Stop action remains the only client lifecycle path that cancels it.
- Rank 3: replay is retained for the bounded 90-minute server horizon and bounded memory capacity; renderer loss reloads a clean paired UI, while whole-process or page-state reconstruction remains an explicitly separate feature rather than a false continuity claim.
