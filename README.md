...
For streamed generations, the gateway owns the upstream request and retains a bounded byte-exact replay buffer. If Android suspends the WebView or its mobile connection drops while the same page remains loaded, the injected same-origin client reconnects at its last delivered byte without starting a second generation. SillyTavern's **Stop** action sends a separate idempotent cancellation request. This does not use PiP, an overlay, a wake lock, or a foreground service. Android process death or a page reload still ends SillyTavern's in-page generation consumer; restoring and saving a generation after that stronger failure requires a separate UI-state recovery feature.
