...
test('background stream continuity requires no visible PiP, overlay, wake lock, or foreground service', async () => {
  const manifest = await text('android/app/src/main/AndroidManifest.xml');
  const source = await text('android/app/src/main/java/app/sillytavern/securemobile/MainActivity.java');

  assert.doesNotMatch(manifest, /supportsPictureInPicture|SYSTEM_ALERT_WINDOW|WAKE_LOCK|FOREGROUND_SERVICE/);
